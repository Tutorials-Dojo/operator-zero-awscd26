...
# execute_remediation, each of which can itself run up to ~150-170s (see
# action-handler's HARNESS_TIMEOUT_SECONDS/read_timeout).
...
# above HARNESS_TIMEOUT_SECONDS. max_attempts=2 allows one retry on a
# transient timeout/connection error; worst case is two full read_timeout
# cycles (~760s), which stays under the Lambda's function timeout (14
# minutes, see CDK) with buffer for the DynamoDB write that follows.
bedrock_agentcore = boto3.client(
    "bedrock-agentcore",
    config=Config(connect_timeout=10, read_timeout=380, retries={"max_attempts": 2}),
)
